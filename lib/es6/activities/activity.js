...
Activity.prototype.getId = function(execContext) {
    return execContext.getActivityId(this);
};
...
Activity.prototype.end = function (callContext, reason, result) {
    ...
    if (callContext) {
        try {
            let bmName = specStrings.activities.createValueCollectedBMName(this.getId(execContext));
...
Activity.prototype.schedule = function (callContext, obj, endCallback) {
    ...
    let execContext = callContext.executionContext;
    let selfId = self.getId(execContext);
        ...
        if (_.isArray(obj) && obj.length || isGenerator) {
            ...
            for (let item of items) {
                if (item instanceof Activity) {
                    scope.__collectValues.push(specStrings.activities.asValueToCollect(item.getId(execContext)));
                    activities.push(item);
                    variables.push(null);
                }
                else if (_.isObject(item) && item.activity instanceof Activity) {
                    scope.__collectValues.push(specStrings.activities.asValueToCollect(item.activity.getId(execContext)));
            ...
            if (activities.length) {
                ...
                for (let i = 0; i < len; i++) {
                    ...
                    bookmarkNames.push(execContext.createBookmark(selfId, specStrings.activities.createValueCollectedBMName(childActivity.getId(execContext)), "resultCollected"));
            ...
            bookmarkNames.push(execContext.createBookmark(selfId, specStrings.activities.createValueCollectedBMName(obj.getId(execContext)), endCallback));
            obj.start(callContext);
        }
        else if (_.isObject(obj) && obj.activity instanceof Activity) {
            bookmarkNames.push(execContext.createBookmark(selfId, specStrings.activities.createValueCollectedBMName(obj.activity.getId(execContext)), endCallback));
