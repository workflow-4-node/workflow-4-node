...
Activity.prototype._children = function* (deep, except, execContext, visited) {
    ...
    let self = this;
    if (!visited.has(self._runtimeId)) {
        visited.add(self._runtimeId);
        ...
        if (_.isArray(obj) && obj.length || isGenerator) {
            ...
            for (let item of items) {
                if (item instanceof Activity) {
                    scope.__collectValues.push(specStrings.activities.asValueToCollect(item));
                    activities.push(item);
                    variables.push(null);
                }
                else if (_.isObject(item) && item.activity instanceof Activity) {
                    scope.__collectValues.push(specStrings.activities.asValueToCollect(item.activity));
            ...
            if (activities.length) {
                ...
                let endBM = (scope.__collectEndBookmarkName = specStrings.activities.createCollectingCompletedBMName(self));
                bookmarkNames.push(execContext.createBookmark(self.instanceId, scope.__collectEndBookmarkName, endCallback));
