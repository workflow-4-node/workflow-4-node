...
Activity.prototype.resultCollected = function (callContext, reason, result, bookmark) {
    ...
    try {
        ...
        if (endWithNoCollectAll) {
            debug("%s: ---- Collecting of values ended, because we're not collecting all values.", selfId);
            ...
            if (onEnd) {
                ...
                if (state.errors.length) {
                    ...
                    finished = function () { execContext.resumeBookmarkInScope(callContext, state.endBookmarkName, Activity.states.fail, error); };
                }
                    ...
                    finished = function () { execContext.resumeBookmarkInScope(callContext, state.endBookmarkName, Activity.states.idle); };
                }
                else {
...
Activity.prototype._scheduleSubActivities = function (callContext, reason, result, endBookmarkName, endCallbackName) {
    ...
    function reg(activity, index, subIndex) {
        ...
        let arrIndex = activities.length;
        activities.push(activity);
        activitiesMap.set(arrIndex, { index: index, subIndex: subIndex });
    }
...
Activity.prototype._subActivitiesGot = function (callContext, reason, result) {
    ...
    let execContext = callContext.executionContext;
    let selfId = callContext.instanceId;
    let state = this.__subActivitySchedulingState;
    ...
    try {
        for (let kvp of state.activitiesMap.entries()) {
            let arrIdx = kvp[0];
            let pos = kvp[1];

            if (_.isNumber(arrIdx) && arrIdx >= 0 && arrIdx < result.length) {
                let value = result[arrIdx];
                if (pos.index === null) {
                    if (finalResult === null) {
                        finalResult = value;
                        continue;
                    ...
                    if (_.isArray(finalResult) && index >= 0 && index < finalResult.length) {
                        if (pos.subIndex === null) {
                            ...
                            let subArray = finalResult[index];
                            let subIndex = pos.subIndex;
