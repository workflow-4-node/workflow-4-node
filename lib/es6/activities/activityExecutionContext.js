...
ActivityExecutionContext.prototype.getActivityId = function(activity, tryIt) {
...
ActivityExecutionContext.prototype.setActivityId = function(activity, id) {
...
ActivityExecutionContext.prototype.appendToContext = function (args) {
    ...
    if (_.isArray(args)) {
        let state = this.getState(this.getActivityId(this._rootActivity));
        for (let arg of args) {
            if (is.activity(arg)) {
                this._initialize(this._rootActivity, arg, c);
                state.childActivityIds.add(this.getActivityId(arg));
            }
        }
    }
    else {
...
ActivityExecutionContext.prototype._initialize = function (parent, activity, idCounter) {
    let activityId = this.getActivityId(activity, true);
    if (!activityId) {
        activityId = (idCounter.instanceId++).toString();
        this.setActivityId(activity, activityId);
    }
    else if (activityId !== (idCounter.instanceId++).toString()) {
        throw new Error("Activity " + activityId + " has been assigned to an other context in a different tree which is not allowed.");
    }

    this._nextActivityId = idCounter.instanceId;
    let state = this.getState(activityId);
    state.parentActivityId = parent ? this.getActivityId(parent) : null;
    ...
    for (let child of activity.immediateChildren(this)) {
        ...
        state.childActivityIds.add(this.getActivityId(child));
    }
};
...
ActivityExecutionContext.prototype.getState = function (id) {
    ...
    if (_.isUndefined(state)) {
        ...
        let activity = null; // Lazy
        state.on(
            enums.ActivityStates.run,
            function () {
                activity = activity || self._knownActivities.get(id);
                if (activity) {
                    self.emit(enums.ActivityStates.run, activity);
                }
                else {
                    throw new errors.ActivityStateExceptionError(`Activity of '${id}' doesn't exists in the context.`);
                }
            });
        state.on(
...
ActivityExecutionContext.prototype._cancelSubtree = function (allIds, activityId) {
    ...
    for (let id of state.childActivityIds.values()) {
        self._cancelSubtree(allIds, id);
    }
...
ActivityExecutionContext.prototype.getStateAndPromotions = function (serializer, enablePromotions) {
    ...
    let scopeStateAndPromotions = this._scopeTree.getState(this, enablePromotions);
