...
ActivityExecutionContext.prototype.appendToContext = function (args) {
    ...
    if (_.isArray(args)) {
        let state = this.getState(this._rootActivity);
        for (let arg of args) {
            if (is.activity(arg)) {
...
ActivityExecutionContext.prototype.getState = function (idOrActivity) {
    let self = this;

    let id, activity;
    if (_.isString(idOrActivity)) {
        id = idOrActivity;
        activity = self._knownActivities.get(id) || null;
    }
    else if (is.activity(idOrActivity)) {
        activity = idOrActivity;
        id = self.getInstanceId(activity);
    }
    else {
