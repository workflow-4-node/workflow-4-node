...
let requireFromRoot = function (resource) {
...
ActivityMarkup.prototype._createActivity = function (types, markup) {
    ...
    let activityAlias = filedNames[0];
...
ActivityMarkup.prototype._createAndInitActivityInstance = function (types, alias, markup) {
    let activity = this._createActivityInstance(types, alias);
    if (!activity) {
    ...
    if (_.isArray(markup)) {
        ...
        if (trimmed.match(/^\s*function\s*\w*\s*\((?:\w+,)*(?:\w+)?\)\s*\{/)) {
            ...
            if (trimmed[0] === "#") {
                ...
                // Get
                let expr = self._createActivityInstance(types, "expression");
                expr.expr = `this.get('${trimmed.substr(1).trim()}')`;
                return expr;
            }
        }
    }
