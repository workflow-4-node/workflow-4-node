...
ResumeBookmark.prototype.run = function (callContext, args) {
    ...
    if (!bookmarkName) callContext.fail(new errors.ValidationError("Bookmark name expected."));
    if (ResumeBookmark.validReasons.indexOf(reason) === -1) callContext.fail(new errors.ValidationError("Reason value '" + reason + "' is not valid."));
