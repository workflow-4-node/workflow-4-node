...
MongoDBPersistence.prototype._ensureIndexes = function () {
    ...
    return Bluebird.all([
        ...
        self._locksCollection.ensureIndex({ activeDelays: 1 }, { w: this._w.w, unique: false }),
        self._stateCollection.ensureIndex(
            { "activeDelays.methodName": 1 },
            {
                w: this._w.w,
                unique: false
            }
...
MongoDBPersistence.prototype.getNextWakeupables = async(function* (count) {
    ...
    let result = yield this._stateCollection
        .aggregate([
            {
                $match: {
                    activeDelays: { $ne: null }
                }
            },
            {
                $project: {
