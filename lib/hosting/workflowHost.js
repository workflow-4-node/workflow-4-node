...
WorkflowHost.prototype._createInstanceAndInvokeMethod = async(
    function (workflowName, methodName, args)
    {
        ...
        var lockInfo = {};
        return await(self._toBeLocked(createInsta, lockInfo));
    });
...
WorkflowHost.prototype._toBeLocked = async(
            ...
            return await(
                asyncHelpers.doInLocked(
                    ...
                    async(
                        function ()
                        {
                            if (lockInfo.id) await(self._persistence.renewLock(lockInfo.id, self._getInLockTimeout()));
                        }),
                    self.options.lockRenewalTimeout));
        }
        finally
...
WorkflowHost.prototype._enterLockForCreatedInstance = function (insta, lockInfo)
{
    var li = await(this._persistence.enterLock(specStrings.hosting.doubleKeys(insta.workflowName, insta.id), this.options.enterLockTimeout, this._getInLockTimeout()));
    lockInfo.id = li.id;
