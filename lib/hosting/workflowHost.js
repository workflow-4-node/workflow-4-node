...
WorkflowHost.prototype.invokeMethod = async(
    function* (workflowName, methodName, args) {
        ...
        self._registry.forEachMethodInfo(workflowName, methodName, function (info) {
            ...
            if (is.defined(tryId)) results.push(
                {
                    info: info,
                    id: tryId
                });
        ...
        if (instanceId) {
            ...
            throw new errors.WorkflowError("Cannot create or continue workflow '" + workflowName + "' be calling method '" + methodName + "'.");
        }
    });
...
WorkflowHost.prototype._createInstanceAndInvokeMethod = async(
    function* (workflow, workflowName, methodName, args) {
        ...
        if (!self._persistence) {
            ...
            var result = yield (insta.create(workflow, methodName, args, lockInfo));
            ...
            try {
                ...
                try {
                    ...
                    if (insta.execState === enums.ActivityStates.idle) {
                        ...
                        if (self.options.lazyPersistence) setTimeout(function () {
                            persistAndUnlock();
                        }, 0); else yield persistAndUnlock();
...
WorkflowHost.prototype._verifyAndRestoreInstanceState = async(
    function* (instanceId, workflowName, methodName, args) {
        ...
        if (self._persistence) {
            try {
                var header = yield (self._persistence.getRunningInstanceIdHeader(workflowName, instanceId));
            ...
            if (!insta) throw new errors.WorkflowError(errorText() + " Inner error: instance " + instanceId + " is unknown.");
        }
