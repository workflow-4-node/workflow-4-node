...
WorkflowHost.prototype.invokeMethod = async(
    function (workflowName, methodName, args)
    {
        ...
        {
            return await(self._invokeMethodOnRunningInstance(runningInstanceIdPath, workflowName, methodName, args));
        }
        else
        {
...
WorkflowHost.prototype._invokeMethodOnRunningInstance = async(
    function (runningInstanceIdPath, workflowName, methodName, args)
    {
        var self = this;
        ...
        var doInvoke = async(
            function (lockInfo)
            {
                var insta = await(self._verifyAndRestoreInstanceState(runningInstanceIdPath.instanceId, workflowName, methodName, args));
                try
                {
        ...
        return await(self._locked(specStrings.hosting.doubleKeys(workflowName, runningInstanceIdPath), doInvoke));
    });
    ...
    function (instanceId, workflowName, methodName, args)
    {
        ...
        if (self._persistence)
        {
            try
            {
                ...
                {
                    return await(self._restoreInstanceState(instanceId, workflowName, header.workflowVersion, header.timestamp));
                }
            }
            catch (e)
            {
