...
function MongoDBPersistence(options)
{
    ...
    this._options = _.extend(
        {
            connectionOptions: { db: { native_parser: true } },
...
MongoDBPersistence.prototype._ensureIndexes = function()
{
    ...
    return Promise.all([
        ...
        new Promise(function(reject, resolve)
        {
            self._locksCollection.ensureIndex({ heldTo: 1 }, { w: 1, unique: true }, function(e)
            {
                if (e) reject(e); else resolve();
            });
        }),
        new Promise(function(reject, resolve)
        {
            self._locksCollection.ensureIndex({ name: 1, heldTo: 1 }, { w: 1, unique: true }, function(e)
            {
                if (e) reject(e); else resolve();
            });
        }),
        ...
        new Promise(function(reject, resolve)
        {
            self._stateCollection.ensureIndex({ workflowName: 1, "idleMethods.methodName": 1 }, { w: 1, unique: true }, function(e)
            {
                if (e) reject(e); else resolve();
            });
        }),
        new Promise(function(reject, resolve)
        {
            self._stateCollection.ensureIndex({ workflowName: 1, instanceId: 1, "idleMethods.methodName": 1 }, { w: 1, unique: true }, function(e)
            {
                if (e) reject(e); else resolve();
            });
        }),
        new Promise(function(reject, resolve)
        {
            self._promotedPropertiesCollection.ensureIndex({ workflowName: 1, instanceId: 1 }, { w: 1, unique: true }, function(e)
            {
                if (e) reject(e); else resolve();
            });
        })
                ...
                self._stateCollection.find({ workflowName: workflowName, "idleMethods.methodName": methodName }, { fields: { idleMethods: 1 }}).toArray(
                    function (e, queryResult)
                    {
                        ...
                        queryResult.forEach(
                            function (s)
                            {
                                fast.forEach(s.idleMethods, function (m)
                                {
            ...
            return new Promise(function(resolve, reject)
            {
                self._stateCollection.findOne(
                    ...
                    function(e, result)
                    {
                        ...
                        {
                            var path;
                            fast.forEach(result.idleMethods,
                                function (m)
                                {
                                    if (m.methodName === methodName)
                                    {
