...
MongoDBPersistence.prototype._getLockById = async(
    function (lockId)
    {
        var cLock = await(this.models.Lock.findById(lockId).lean().execQ());
        var now = new Date();
...
MongoDBPersistence.prototype.getRunningInstanceIdPaths = async(
    function (workflowName, methodName)
    {
        ...
        var queryResult =
            await(
                this.models.State.find({ workflowName: workflowName, "idleMethods.methodName": methodName })
                    .select("idleMethods")
                    .lean()
                    .execQ());
...
MongoDBPersistence.prototype.persistState = async(
    function (state)
    {
        ...
        await(
            [
                this.models.State.findOneAndUpdate(
                    {
                ...
                this.models.PromotedProperties.findOneAndUpdate(
                    {
                        workflowName: state.workflowName,
                        instanceId: state.instanceId
                    },
        ...
        var result =
            await(
                this.models.State.findOne(
                    ...
                    }).select("idleMethods updatedOn workflowVersion").lean().execQ());
