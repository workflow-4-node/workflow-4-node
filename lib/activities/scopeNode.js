...
function ScopeNode(id, scopePart)
{
    ...
    this.keys = [];
    for (var key in scopePart) this.keys.push(key);
}
...
ScopeNode.prototype.setPropertyValue = function (name, value, canSetPrivate)
{
    if (this._isPrivate(name))
    {
        if (canSetPrivate)
        {
            if (!is.defined(this._scopePart[name])) this.keys.push(name);
...
ScopeNode.prototype.deleteProperty = function (name, canDeletePrivate)
{
    if (is.defined(this._scopePart[name]))
    {
        if (this._isPrivate(name))
        {
            if (canDeletePrivate)
            {
                this.keys.splice(fast.indexOf(this.keys, name), 1);
...
{
    var self = this;
    fast.forEach(self.keys, function(fn)
    {
