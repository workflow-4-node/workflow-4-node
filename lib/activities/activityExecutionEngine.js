                ...
                try
                {
                    switch (reason)
                    {
                        ...
                        case Activity.states.cancel:
                            defer.reject(new errors.Cancelled());
                            break;
                        case Activity.states.idle:
                            defer.reject(new errors.Idle());
                            break;
                        default :
...
ActivityExecutionEngine.prototype.resumeBookmark = function (name, reason, result)
{
    ...
    try
    {
                    ...
                    try
                    {
                        if (reason === enums.ActivityStates.complete || reason === enums.ActivityStates.idle)
                        {
                            if (self._context.isBookmarkExists(name))
                            {
                                ...
                                {
                                    defer.reject(new errors.ActivityRuntimeError("Workflow has been completed before bookmark '" + name + "' reached."));
                                }
                                else
                                {
                                    defer.reject(new errors.Idle("Workflow has been completed before bookmark '" + name + "' reached."));
                                }
                            }
                            else
                            {
                        ...
                        {
                            defer.reject(new errors.ActivityRuntimeError("Workflow has been cancelled before bookmark '" + name + "' reached."));
                        }
        ...
        {
            throw new errors.ActivityRuntimeError("Cannot resume bookmark, while the workflow is not in the idle state.");
        }
    }
    catch (e)
    {
