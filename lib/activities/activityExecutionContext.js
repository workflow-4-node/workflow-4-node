...
/* SERIALIZATION */
ActivityExecutionContext.prototype.asJSON = function()
{
    var activityStates = new StrMap();
    this._activityStates.forEach(function(s)
    {
        activityStates.add(d.activityId, s.asJSON());
    })
    ...
    return {
        ...
        scopeTree: this._scopeTree.asJSON()
    }
}

ActivityExecutionContext.prototype.fromJSON = function(json)
{
    ...
    this._scopeTree.fromJSON(json.scopeTree);
}
/* SERIALIZATION */
...
{
    if (this._bookmarks.get(bookmark.name)) throw new ex.ActivityRuntimeError("Bookmark '" + bookmark.name + "' already exists.");
