...
Activity.prototype.start = function (context)
{
    ...
    context.beginScope(self.id, self.createScopePart());
    scopeHasBegun = true;

    self.emit(context, Activity.states.run);
    eventEmited = true;
    self.run.call(context.scope, context, args);
...
Activity.prototype.createScopePart = function ()
{
    ...
    fast.forEach(self.getKeys(), function(fieldName)
    {
        if (self.nonScopedProperties.exists(fieldName)) return;
        if (Activity.prototype[fieldName]) return;
        scopePart[fieldName] = self[fieldName];
    });
    return scopePart;
}
