...
Activity.prototype.start = function (callContext)
{
    ...
    var state = callContext.executionContext.getState(self.id);
    ...
    self.emit(callContext, Activity.states.run);

    var myCallContext = callContext.next(this);
...
Activity.prototype.end = function (callContext, reason, result)
{
    ...
    var parentCallContext = callContext.back(inIdle);
    var execContext = parentCallContext.executionContext;
    ...
    if (parentCallContext)
    {
        ...
        {
            emit();
            execContext.resumeBookmarkInScope(bmName, reason, result);
            return;
        }
    }
    else
    {
        ...
        // then instead of emmitting done, we have to continue them.
        if (inIdle && execContext.context.processResumeBookmarkQueue())
        {
...
Activity.prototype.schedule = function (callContext, obj, endCallback)
{
    ...
    if (Array.isArray(obj) && obj.length)
    {
        ...
        if (activities.length)
        {
            ...
            fast.forEach(activities,
                function (a)
                {
                    execContext.createBookmark(self.id, specStrings.activities.createValueCollectedBMName(a), "resultCollected");
                    a.start(callContext);
                });
        }
        else
...
Activity.prototype.emit = function (callContext)
{
    var state = callContext.executionContext.getState(this.id);
