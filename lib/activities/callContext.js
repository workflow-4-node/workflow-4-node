...
CallContext.prototype.next = function(childActivityOrActivityId)
{
    ...
    return new CallContext(
        ...
        child,
        this._scopeTree.next(this.activityId, child.id, child.createScopePart()));
}
...
CallContext.prototype.back = function(keepScope)
{
    ...
    if (parentId)
    {
        return new CallContext(
            this._executionContext,
            parentId,
            this._scopeTree.back(this.activityId, keepScope));
    }
    else
    {
        return null;
...
{
    this._executionContext.resumeBookmarkInternal(name, reason, result);
}
