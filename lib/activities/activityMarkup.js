...
requireFromRoot = (function(root) {
    return function(resource) {
        ...
        for (var i = 0; i < splits.length; i++) {
            var rp = path.resolve(root).split('/node_modules')[i];
            rp = path.join(rp, resource);
            if (fs.existsSync(rp)) return require(rp);
        }
        return require(path.resolve(path.join(root, "../../", resource))); // Unit tests
    }
})(__dirname);
...
ActivityMarkup.prototype._registerSystemTypes = function () {
    ...
    this._registerType("./if");
}

ActivityMarkup.prototype._registerType = function (sourcePath) {
    this._registerTypeTo(this._systemTypes, sourcePath);
}

ActivityMarkup.prototype._registerTypeTo = function (types, sourcePath, fromRoot) {
    var type = fromRoot ? requireFromRoot(sourcePath) : require(sourcePath);
    ...
    if (_.isArray(pars)) {
        ...
        for (var fieldName in pars) {
            if (fieldName == "args") {
                ...
                // Require:
                self._register(types, pars[fieldName]);
            }
            else {
...
ActivityMarkup.prototype._register = function(types, markup) {
    var self = this;

    if (_.isArray(markup)) {
        markup.forEach(function(item) {
            self._register(types, item);
        });
    }
    else if (_.isString(markup)) {
        self._registerTypeTo(types, markup, true);
    }
    else {
