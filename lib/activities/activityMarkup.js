    ...
    if (_.isArray(pars)) {
        ...
        for (var fieldName in pars) {
            if (fieldName == "args") {
                ...
                activity[fieldName] = self._createValue(types, pars[fieldName], true, is.template(activity));
            }
        }
        if (to) {
...
ActivityMarkup.prototype._createValue = function (types, markup, canBeArray, noTemplate) {
    var self = this;
