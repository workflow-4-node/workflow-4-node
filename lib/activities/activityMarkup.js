...
ActivityMarkup.prototype._createActivity = function (markup)
{
    ...
    return this._createAndInitActivityInstance(activityAlias, markup);
}

ActivityMarkup.prototype._createAndInitActivityInstance = function (alias, markup)
{
    var activity = this._createActivityInstance(alias);
    ...
    if (pars) this._setupActivity(activityRef, pars);
    return activityRef.value;
}

ActivityMarkup.prototype._createActivityInstance = function (alias)
{
    var type = this._types.get(alias);
    if (is.undefined(type)) return null;
    return new type();
}

ActivityMarkup.prototype._setupActivity = function (activityRef, pars)
{
    var self = this;
    ...
    else if (_.isObject(pars))
    {
        ...
        for (var fieldName in pars)
        {
            if (fieldName == "args")
            {
                var v = self._createValue(pars[fieldName], true);
                if (!_.isArray(v)) v = [v];
                activity.args = v;
            }
            ...
            else if (fieldName[0] == "!")
            {
                ...
                activity.promoted(fieldName.substr(1), self._createValue(pars[fieldName], true));
            }
            else
            {
                activity[fieldName] = self._createValue(pars[fieldName], true);
            }
        }
        if (to)
        {
            var current = activity;
            var assign = activityRef.value = this._createActivityInstance("assign");
    ...
    {
        // 1 arg
        activity.args = [ self._createValue(pars) ];
    }
}
...
ActivityMarkup.prototype._createValue = function (markup, canBeArray)
{
    ...
    else if (_.isObject(markup))
    {
        ...
        if (filedNames.length == 1)
        {
            ...
            if (is.defined(self._types.containsKey(fieldName)))
            {
                // Activity:
                return self._createAndInitActivityInstance(fieldName, markup);
            }
        }
    }
    else if (_.isString(markup))
    {
        ...
        {
            // Expression
            var expr = self._createActivityInstance("expression");
            expr.expr = trimmed.substr(1, trimmed.length - 2);
            return expr;
        }
    }
