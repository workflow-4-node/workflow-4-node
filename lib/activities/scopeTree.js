...
ScopeTree.prototype.asJSON = function()
{
    ...
    if (!self.isOnInitial) throw new Error("Cannot generate JSON state of state tree if it's not in initial state.");
    ...
    self._nodes.forEachValue(function(node)
    {
        ...
        node.forEachPart(function(part)
        {
                ...
                _.isFunction(activity[part.name]))
            {
                item.parts.push(
                    {
                        name: specStrings.hosting.createActivityMethod(part.name),
                        value: null
                    });
            }
            else
    ...
    try
    {
        json.forEach(
            function (item)
            {
                ...
                {
                    var activityMethod = specStrings.hosting.getActivityMethodName(part.name);
                    if (activityMethod)
                    {
                        if (!_.ssFunction(scopePart[part.name] = activity[activityMethod]))
