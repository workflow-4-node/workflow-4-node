    ...
    self._nodes.forEachValue(
        function (node)
        {
            ...
            node.forEachPart(
                function (part)
                {
                    if (part.value instanceof Activity)
                    {
                        item.parts.push(
                            {
                                name: part.name,
                                value: specStrings.hosting.createActivityInstancePart(part.value)
                            });
                    }
                    else if (_.isFunction(part.value) && !activity.hasOwnProperty(part.name) &&
                        _.isFunction(activity[part.name]))
                    {
                        item.parts.push(specStrings.hosting.createActivityMethodPart(part.name));
                    }
                    else if (!activity.nonSerializedProperties.exists(part.name))
                    {
            ...
            if (activity.promotedProperties)
            {
                activity.promotedProperties.forEach(
                    ...
                    {
                        var pv = node.getPartValue(promotedPropName);
                        if (pv !== undefined && !(pv instanceof Activity))
                        {
                            var promotedEntry = promotions.get(promotedPropName);
                            ...
                            {
                                promotions.add(promotedPropName, { level: node.id, value: pv });
                            }
                        }
                    });
            }
            ...
            function (item)
            {
                ...
                item.parts.forEach(
                    function (part)
                    {
                        ...
                        if (activityMethod)
                        {
                            if (!_.isFunction(scopePart[activityMethod] = activity[activityMethod]))
                                throw new Error("Activity has no function '" + part + "'.");
                        }
                        else
                        {
