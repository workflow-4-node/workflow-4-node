...
/* SERIALIZATION */
ScopeTree.prototype.getStateAndPromotions = function ()
{
    var self = this;
    ...
    var state = [];
    var promotedProperties = new StrMap();
    self._nodes.forEachValue(
        function (node)
        {
            ...
            node.forEachFieldValue(
                function (part)
                {
                    if (!activity.nonSerializedProperties.exists(part.name))
                    {
                        ...
                        else if (is.activity(part.value))
                        {
                            item.parts.push(
                                {
                                    name: part.name,
                                    value: specStrings.hosting.createActivityInstancePart(part.value.id)
                                });
                        }
                        else if (_.isFunction(part.value) && !activity.hasOwnProperty(part.name) &&
                            _.isFunction(activity[part.name]))
                        {
                            item.parts.push(specStrings.hosting.createActivityPropertyPart(part.name));
                        }
                        else if (_.isObject(part.value) && part.value === activity[part.name])
                        {
                            item.parts.push(specStrings.hosting.createActivityPropertyPart(part.name));
                        }
                        else
            ...
            // Promotions:
            if (activity.promotedProperties)
            {
                activity.promotedProperties.forEach(
                    function (promotedPropName)
                    {
                        var pv = node.getFieldValue(promotedPropName);
                        if (pv !== undefined && !(is.activity(pv)))
                        {
                            ...
                            if (promotedEntry === undefined || node.id > promotedEntry.level)
                            {
...
ScopeTree.prototype.setState = function (json)
{
    ...
    {
        throw new errors.WorkflowError("Cannot restore state tree, because data is corrupt. Inner error: " + e.toString());
    }
}
/* SERIALIZATION */
...
ScopeTree.prototype._leaveCurrentScope = function ()
{
    ...
    for (var fn in this.currentScope)
    {
        ...
        {
            var isPrivate = fn[0] === "_";
            if (isPrivate)
            {
                if (this._currentNode.isFieldExists(fn))
                {
            ...
            else if (this._currentNode.isFieldExists(fn))
            {
                this._currentNode.updateField(fn, this.currentScope[fn]);
            }
            else if (this._currentNode.parent != this._initialNode)
            {
                var found = false;
                this._currentNode.parent.forEachToRoot(function(node)
                {
                    if (node.isFieldExists(fn))
                    {
                        found = true;
                        return false;
                    }
                });
                if (!found) this._currentNode.addField(fn, this.currentScope[fn]);
            }
        }
    }
}
