...
Declarator.prototype.run = function (context, args)
{
    ...
    fast.forEach(self.activity.getKeys(), function (fieldName)
    {
        if (is.undefined(self.activity.constructor.prototype[fieldName]) &&
            !self.activity.reservedProperties.exists(fieldName) &&
            !self.activity.nonScopedProperties.exists(fieldName))
        {
    ...
    if (activityVariables.length)
    {
        ...
        self.schedule(activityVariables, "_varsGot");
    }
    else
    {
